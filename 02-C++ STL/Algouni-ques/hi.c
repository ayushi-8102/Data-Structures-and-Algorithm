...
#define ALPHA_AFFINE_KEY 11 // alpha key for affine cypher
#define BETA_AFFINE_KEY 15  // beta key for affine cypher
...
// driven function for encrypt array
...
// driven function for decrypt array
void decrypt(char str[], char keyT[5][5], int ps);
// function for decrypt inputs(character array needs to convert, generated key matrix , converted plain text lenght)

// function for Ceaser cypher encryptation and decryptation
void encryptbyCeaser(char *ch, char *res, int key); 
// function for encrypt inputs by Ceaser Cypher(pointer to  the character needs to convert, converted plain text, key = 3)
...
// function for encrypt inputs by Affine Cypher(character array needs to convert, alpha key =7, beta key =5)
void *decryptByAffinefairstr(char str[], int a, int b); 
// function for decrypt inputs by Affine Cypher(character array needs to convert, alpha key =7, beta key =5)
...
void main()
{
	...
	printf("Affine Cypher encrypted text with alpha key(11) and beta key (15): %s \n", str);
...
decryptByCeaser(str, str, CEASER_KEY);
	printf("Ceaser Cypher decrpyted text with key (12): %s \n", str);
